...
import { dateIsoInTz, defaultSlotFor, hmInTz, isValidTimezone, parseZonedDateTime } from "./time";
...
export async function scheduleItem(args: {
  ...
  if (!isValidTimezone(args.timezone)) return { ok: false, reason: "invalid_timezone", message: "Choose a valid workspace timezone." };
  if (args.timeStr && !/^([01]\d|2[0-3]):[0-5]\d$/.test(args.timeStr)) return { ok: false, reason: "invalid_time", message: "Time must be a valid HH:mm value." };
